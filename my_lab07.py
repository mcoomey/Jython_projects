...
#----------------------------------------------------------
def RotateCounterClockwise(Canvas):
    newWidth = getHeight(Canvas)
    newHeight = getWidth(Canvas)
    NewCanvas = makeEmptyPicture(newWidth, newHeight)
    for X in range(newWidth-1):
        for Y in range(newHeight-1):
            oldPX = getPixel(Canvas, newHeight-Y-1, X)
            color = getColor(oldPX)
            newPX = getPixel(NewCanvas, X, Y)
    ...
    return(NewCanvas)
#----------------------------------------------------------
def RotateClockwise(Canvas):
    newWidth = getHeight(Canvas)
    newHeight = getWidth(Canvas)
    NewCanvas = makeEmptyPicture(newWidth, newHeight)
    for X in range(newWidth-1):
        for Y in range(newHeight-1):
            oldPX = getPixel(Canvas, Y, newWidth-X-1)
            color = getColor(oldPX)
            newPX = getPixel(NewCanvas, X, Y)
            setColor(newPX, color)
    Canvas.hide()
    ...
    if Canvas != None:
        ...
        while (not Done):
            ...
            if Input != None:           # only process input -- ignore clicked 'cancel'
                ...
                elif (Command == "["): Canvas = RotateCounterClockwise(Canvas)
                elif (Command == "]"): Canvas = RotateClockwise(Canvas)
